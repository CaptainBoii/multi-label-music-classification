...
for genre in genres:
    ...
    for spect_type in spect_types:
        ...
        model_checkpoint_callback = ModelCheckpoint(
            filepath=save_dir + spect_type + genre + "-{epoch:02d}-{val_accuracy:.2f}-{val_loss:.2f}.keras",
            monitor='val_accuracy',
            mode='max',
            save_best_only=True)

        early_stopping = EarlyStopping(monitor='val_loss', patience=25, verbose=0, min_delta=1e-4)

        l1l2 = L1L2(l1=0.05, l2=0.05)

        optimizer = Adam(learning_rate=0.003, use_ema=True, ema_momentum=0.7)
        model = Sequential([
